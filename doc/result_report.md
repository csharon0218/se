...
| 1.3 | 2026-06-11 | 최민서 | 조원 리뷰 반영: 예외 케이스 결과 라벨을 EXPECTED FAIL / 정상 차단으로 통일 | 수정 |
...
5. 테스트 결과를 SUCCESS, EXPECTED FAIL / 정상 차단, YES, PARTIAL 상태로 정리한다.
...
| FAIL | 실제 수행 결과가 기대 결과와 다르게 동작하여 결함으로 판단되는 상태 |
| EXPECTED FAIL / 정상 차단 | 잘못된 입력, 권한 없음, 정책 위반, 만료 링크 접근 등 예외 조건이 시스템에 의해 의도대로 거절·차단된 상태 |
...
> 본 보고서에서는 잘못된 입력, 권한 없는 접근, 만료 링크 접근처럼 시스템이 의도적으로 거절해야 하는 예외 케이스를 `FAIL`로 표기하지 않고 `EXPECTED FAIL / 정상 차단`으로 표기한다. 이를 통해 실제 결함으로 인한 실패와 정상적인 예외 처리를 구분한다.
...
| T2 | FR-001 |  | Sync1234! | minseo01@kau.ac.kr | EXPECTED FAIL / 정상 차단 |
| T3 | FR-001 | minseo01 |  | minseo01@kau.ac.kr | EXPECTED FAIL / 정상 차단 |
| T4 | FR-001 | minseo02 | Sync1234! | email-format-error | EXPECTED FAIL / 정상 차단 |
| T5 | FR-004 | existingUser | Sync1234! | existing@kau.ac.kr | EXPECTED FAIL / 정상 차단 |
...
| T7 | FR-002 | minseo01 | wrongPassword | EXPECTED FAIL / 정상 차단 |
| T8 | FR-002 | unknownUser | Sync1234! | EXPECTED FAIL / 정상 차단 |
| T9 | FR-003 | 로그인하지 않은 사용자 | - | EXPECTED FAIL / 정상 차단: 주요 기능 접근 차단 |
| T10 | FR-003 | 비활성화 계정 | Sync1234! | EXPECTED FAIL / 정상 차단 |
...
| T13 | FR-006 | script.exe | 허용되지 않은 파일 형식 | EXPECTED FAIL / 정상 차단 |
...
| T17 | FR-009 | dataset.zip | 사용자 저장 공간 한도 초과 | EXPECTED FAIL / 정상 차단 |
...
| T19 | FR-007 | report.pdf | 다운로드 권한 없음 | EXPECTED FAIL / 정상 차단 |
| T20 | FR-007 | deleted_report.pdf | 파일 원본 없음 | EXPECTED FAIL / 정상 차단 |
...
| T28 | FR-012 | 동일 경로에 같은 이름의 폴더를 생성한다. | EXPECTED FAIL / 정상 차단 |
...
| T37 | FR-024 | 존재하지 않는 사용자에게 공유한다. | EXPECTED FAIL / 정상 차단 |
| T38 | FR-025 | 권한 수준을 선택하지 않고 공유한다. | EXPECTED FAIL / 정상 차단 |
| T39 | NFR-012 | 공유 권한 없는 사용자가 공유를 시도한다. | EXPECTED FAIL / 정상 차단 |
...
| T43 | FR-029, NFR-013 | 만료된 외부 링크로 접근한다. | EXPECTED FAIL / 정상 차단 |
| T44 | FR-028 | 다운로드 미허용 링크에서 다운로드를 시도한다. | EXPECTED FAIL / 정상 차단 |
...
| T48 | FR-030 | 이전 버전이 없는 파일의 버전 목록을 조회한다. | EXPECTED FAIL / 정상 차단 |
| T49 | FR-031 | 권한 없는 사용자가 버전을 복원한다. | EXPECTED FAIL / 정상 차단 |
...
| T51 | FR-032 | 복구 가능 기간이 지난 파일을 복구한다. | EXPECTED FAIL / 정상 차단 |
| T52 | FR-032 | 원래 폴더가 삭제된 파일을 복구한다. | SUCCESS: 복구 위치 선택 화면 제공 |
| T53 | FR-032 | 권한 없는 사용자가 삭제 파일을 복구한다. | EXPECTED FAIL / 정상 차단 |
...
| T59 | FR-033 | 일반 사용자가 관리자 화면에 접근한다. | EXPECTED FAIL / 정상 차단 |
...
| 5단계 | 성공 결과, EXPECTED FAIL / 정상 차단 결과, 실제 결함 결과를 구분한다. | 예상 결과 |
...
| 테스트 통과 기준 | 정상 흐름은 SUCCESS 또는 YES, 예외 흐름은 EXPECTED FAIL / 정상 차단으로 기록하며, 기대 결과와 다르게 동작한 경우에만 FAIL로 기록한다. |
...
| 테스트 종료 기준 | 모든 테스트 케이스의 결과가 SUCCESS, YES, EXPECTED FAIL / 정상 차단, PARTIAL, BLOCKED, FAIL 중 하나로 기록되고, Critical 결함이 남아 있지 않아야 한다. |
...
> 위 표의 `정상 차단`은 테스트 실패가 아니라, 잘못된 입력·권한 없음·정책 위반·만료 링크 접근 등이 시스템에 의해 기대한 대로 차단되었음을 의미한다. 따라서 본 보고서의 예외 케이스 결과는 `FAIL`이 아닌 `EXPECTED FAIL / 정상 차단`으로 표기한다. `부분 보완`은 핵심 기능은 충족하지만 실제 구현 후 UI 또는 호환성 점검이 필요한 항목을 의미한다.
...
| T2 |  | Sync1234! | minseo01@kau.ac.kr | EXPECTED FAIL / 정상 차단 |
| T3 | minseo01 |  | minseo01@kau.ac.kr | EXPECTED FAIL / 정상 차단 |
| T4 | minseo02 | Sync1234! | email-format-error | EXPECTED FAIL / 정상 차단 |
| T5 | existingUser | Sync1234! | existing@kau.ac.kr | EXPECTED FAIL / 정상 차단 |

회원가입 기능은 필수 입력값 누락, 이메일 형식 오류, 중복 계정에 대해 모두 EXPECTED FAIL / 정상 차단으로 처리되는 것으로 검증하였다.
...
| T7 | minseo01 | wrongPassword | EXPECTED FAIL / 정상 차단 |
| T8 | unknownUser | Sync1234! | EXPECTED FAIL / 정상 차단 |
| T9 | 로그인하지 않은 사용자 | - | EXPECTED FAIL / 정상 차단 |
| T10 | 비활성화 계정 | Sync1234! | EXPECTED FAIL / 정상 차단 |

로그인 기능은 올바른 사용자만 로그인에 성공하고, 로그인하지 않은 사용자 또는 비활성화 계정은 EXPECTED FAIL / 정상 차단으로 처리되는 것으로 검증하였다.
...
| T13 | 허용되지 않은 실행 파일 업로드 | EXPECTED FAIL / 정상 차단 |
| T14 | 업로드 성공 후 메타데이터 저장 | SUCCESS |
| T15 | 150MB 대용량 파일 업로드 및 진행률 표시 | SUCCESS |
| T16 | 업로드 중단 후 이어올리기 | SUCCESS |
| T17 | 저장 공간 한도 초과 파일 업로드 | EXPECTED FAIL / 정상 차단 |

허용된 파일은 업로드에 성공하고, 허용되지 않은 형식 또는 저장 공간 초과 조건에서는 EXPECTED FAIL / 정상 차단으로 업로드가 제한되는 것으로 검증하였다.
...
| T19 | 다운로드 권한이 없는 사용자의 파일 다운로드 | EXPECTED FAIL / 정상 차단 |
| T20 | 파일 원본이 없는 파일 다운로드 | EXPECTED FAIL / 정상 차단 |
| T21 | 다운로드 요청 후 3초 이내 시작 | SUCCESS |

다운로드 기능은 권한이 있는 사용자만 수행 가능하며, 권한이 없거나 파일 원본이 없는 경우 EXPECTED FAIL / 정상 차단으로 처리되는 것으로 검증하였다.
...
| T28 | 동일 경로 중복 폴더 생성 | EXPECTED FAIL / 정상 차단 |

폴더 및 파일 관리 기능은 권한이 있는 사용자에게 정상 동작하고, 중복 폴더 생성은 EXPECTED FAIL / 정상 차단으로 제한되는 것으로 검증하였다.
...
| T37 | 존재하지 않는 사용자에게 공유 | EXPECTED FAIL / 정상 차단 |
| T38 | 권한 수준 미선택 상태로 공유 | EXPECTED FAIL / 정상 차단 |
| T39 | 공유 권한 없는 사용자의 공유 시도 | EXPECTED FAIL / 정상 차단 |

내부 공유 기능은 존재하는 사용자와 적절한 권한 수준이 선택된 경우에만 성공하며, 존재하지 않는 사용자나 권한 미선택 조건은 EXPECTED FAIL / 정상 차단으로 처리되는 것으로 검증하였다.
...
| T43 | 만료된 외부 링크 접근 | EXPECTED FAIL / 정상 차단 |
| T44 | 다운로드 미허용 링크에서 다운로드 시도 | EXPECTED FAIL / 정상 차단 |
| T45 | 유효한 외부 링크 미리보기 요청 | SUCCESS |

외부 링크 기능은 유효한 링크와 정책을 만족하는 조건에서만 접근을 허용하며, 만료 링크와 다운로드 미허용 조건은 EXPECTED FAIL / 정상 차단으로 제한되는 것으로 검증하였다.
...
| T48 | 이전 버전이 없는 파일의 버전 목록 조회 | EXPECTED FAIL / 정상 차단 |
| T49 | 권한 없는 사용자의 버전 복원 | EXPECTED FAIL / 정상 차단 |

파일 버전 관리는 이전 버전이 존재하고 접근 권한이 있는 경우에만 복원이 가능하며, 이전 버전 없음 또는 권한 없음 조건은 EXPECTED FAIL / 정상 차단으로 처리되는 것으로 검증하였다.
...
| T51 | 복구 가능 기간이 지난 파일 복구 | EXPECTED FAIL / 정상 차단 |
| T52 | 원래 폴더가 삭제된 파일 복구 | SUCCESS: 복구 위치 선택 화면 제공 |
| T53 | 권한 없는 사용자의 삭제 파일 복구 | EXPECTED FAIL / 정상 차단 |

삭제 파일 복구 기능은 복구 가능 기간 내 파일에 대해서만 복구를 허용하며, 복구 기간 만료 또는 권한 없음 조건은 EXPECTED FAIL / 정상 차단으로 처리되는 것으로 검증하였다. 원래 위치가 없는 경우에는 복구 위치 선택 화면을 제공하는 것으로 검증하였다.
...
| T59 | 일반 사용자 관리자 화면 접근 | EXPECTED FAIL / 정상 차단 |

관리자 기능은 관리자 권한을 가진 사용자에게만 허용되며, 일반 사용자는 관리자 화면 접근이 EXPECTED FAIL / 정상 차단으로 차단되는 것으로 검증하였다.
...
본 절은 조원 리뷰를 통해 확인된 개선 의견과 반영 내용을 정리한다. 리뷰에서 `FAIL` 라벨이 실제 기능 실패와 정상 차단을 동시에 의미할 수 있어 결과 해석이 혼동될 수 있다는 의견이 제시되었다. 이에 따라 예외 케이스의 결과 표현을 `EXPECTED FAIL / 정상 차단`으로 통일하고, 실제 결함 상태인 `FAIL`과 구분하였다.

| 리뷰 내용 | 반영 여부 | 반영 내용 |
|---|---|---|
| `FAIL` 라벨이 실제 기능 실패와 정상 차단을 모두 의미할 수 있어 결과 표만 보면 결함인지 정상 동작인지 구분하기 어렵다. | 반영 | 예외 케이스의 결과값을 `EXPECTED FAIL / 정상 차단`으로 통일하고, `FAIL`은 실제 결함 발생 시 사용하는 상태로 재정의하였다. |
| T19 권한 없는 다운로드, T43 만료 링크 접근 등 예외 케이스는 정상적으로 거절되어야 하는 케이스이므로 `EXPECTED FAIL` 또는 `정상 차단`으로 표기하면 좋다. | 반영 | 회원가입, 로그인, 업로드, 다운로드, 공유, 외부 링크, 버전 관리, 삭제 파일 복구, 관리자 기능의 예외 케이스 결과 표기를 `EXPECTED FAIL / 정상 차단`으로 수정하였다. |
| 구현 이후 실측 테스트 시 실제 결함과 의도된 거절을 구분할 수 있도록 결과 상태 정의를 명확히 하면 좋다. | 반영 | 2.4 테스트 결과 상태 정의에서 `FAIL`과 `EXPECTED FAIL / 정상 차단`의 의미를 분리하여 작성하였다. |
