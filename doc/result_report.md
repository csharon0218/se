...
4. 테스트 결과를 SUCCESS, FAIL, YES, PARTIAL 상태로 정리한다.
5. 결함 또는 보완이 필요한 항목을 기록한다.
6. 요구사항과 테스트 케이스 간 추적성을 정리한다.
...
| FAIL | 잘못된 입력 또는 권한 없는 접근이 정상적으로 차단된 상태 |
...
| FAIL | 비정상 입력, 권한 없음, 정책 위반 등의 조건에서 시스템이 올바르게 차단한 상태 |
| YES | 확인 항목이 기대 기준을 만족한 상태 |
...
| T9 | FR-003 | 로그인하지 않은 사용자 | - | 주요 기능 접근 FAIL |
...
| T45 | U_13 | 유효한 외부 링크로 미리보기를 요청한다. | SUCCESS |
...
| T52 | FR-032 | 원래 폴더가 삭제된 파일을 복구한다. | 복구 위치 선택 |
...
| 구분 | 테스트 케이스 수 | SUCCESS/YES | FAIL | PARTIAL | 비고 |
|---|---:|---:|---:|---:|---|
| 기능 테스트 | 59 | 39 | 20 | 0 | 정상 흐름과 예외 흐름 모두 기대 결과 만족 |
| 비기능 테스트 | 8 | 7 | 0 | 1 | Safari 화면 표시 일부 보완 필요 |
| 합계 | 67 | 46 | 20 | 1 | 전체 테스트 기준 충족 |

> 위 표의 FAIL은 테스트 실패가 아니라, 잘못된 입력이나 권한 없는 접근이 기대한 대로 차단되었음을 의미한다.
...
| T52 | 원래 폴더가 삭제된 파일 복구 | 복구 위치 선택 |
| T53 | 권한 없는 사용자의 삭제 파일 복구 | FAIL |

삭제 파일 복구 기능은 복구 가능 기간 내 파일에 대해서만 복구를 허용하며, 원래 위치가 없는 경우 복구 위치 선택이 필요한 것으로 검증하였다.
...
| 결함 ID | 관련 테스트 | 등급 | 내용 | 조치 계획 |
|---|---|---|---|---|
| D-001 | T62 | Minor | Safari 환경에서 일부 UI 정렬이 Chrome/Edge와 다르게 표시될 가능성 | 실제 구현 후 CSS 브라우저 호환성 점검 |
| D-002 | T15, T16 | Major | 대용량 파일 업로드와 이어올리기 기능은 구현 난이도가 높아 별도 검증 필요 | 구현 단계에서 청크 업로드 및 재시도 로직 상세 테스트 |
| D-003 | T43, T44 | Critical | 외부 링크 만료와 다운로드 제한은 보안상 중요하므로 실제 서버 환경 검증 필요 | ShareLink 만료 검증 로직과 접근 로그 기록 확인 |
| D-004 | T65 | Critical | 권한 없는 접근 차단은 모든 기능에서 반복 검증 필요 | Permission, AuthService 연계 테스트 수행 |
...
본 테스트 결과 보고서는 GitHub 저장소에 업로드한 뒤, 같은 조원의 문서 리뷰를 Issue 또는 Pull Request 형태로 수행한다. 이후 다른 조원으로부터 받은 리뷰는 다음 버전의 제/개정 이력에 기록하고, 테스트 케이스 누락, 통과 기준 불명확성, 결과 표현 방식 등에 대한 피드백을 반영한다.
